...
def main():
    ...
    # Common arguments
    parser.add_argument("--z_dim", type=int, default=10, help="Dimension of latent space (10/modality × 1 modality)")
    ...
    # New arguments for multimodal approach (WAVEFORM ONLY)
    ...
    # Output directory — overrides the default relative path so container jobs can
    # write to a predictable location that upload_smoke_results.py can find.
    parser.add_argument("--output-dir", "--output_dir", type=str, default=None,
                        help="Root directory for results CSVs and checkpoints. "
                             "If unset, defaults to ../results_hippie_transductive/<dataset>/...")
    ...
    # Load target dataset
    # -------------------------------
    # Load the actual data (WAVEFORM ONLY)
    ...
    # Sanity check
    assert len(supervised_wf) == len(supervised_labels), "Data and labels must have the same number of samples"
    ...
    # Define modalities and weights (WAVEFORM ONLY)
    modalities = {
        "wave": 50
    }
    modality_weights = {
    ...
    all_dataset_files = {
        ...
        "dandi_000041_cell_type": 9,
        "ibl_brainwide_good": 10,
    }

    num_sources = max(all_dataset_files.values()) + 1

    # Load ALL datasets for pretraining (multi-dataset learning)
    print("\n" + "="*60)
    print("Loading ALL datasets for multi-dataset pretraining...")
    print("="*60)

    datasets_pretrain = []
    all_source_labels = []

    for dataset_name, source_id in all_dataset_files.items():
        try:
            # Load dataset (WAVEFORM ONLY)
            ...
            # Create multimodal dataset for this source (WAVEFORM ONLY)
            data_dict = {
                "wave": dataset_wf
            }
    ...
    # Load target dataset for fine-tuning (WAVEFORM ONLY)
    ...
    _default_results_dir = (
        f"../results_hippie_transductive/{args.dataset}/fold_{args.cv_fold}"
        f"/config_{args.config}_zdim_{args.z_dim}_B_{args.beta}"
    )
    results_dir = args.output_dir if args.output_dir else _default_results_dir
