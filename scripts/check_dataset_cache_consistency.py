...
# Differences that the Methods already accounts for: a cache legitimately holds
# fewer units than labels.csv when unlabelled units or rare classes are dropped.
# Each entry is (dataset, expected_cache_n, reason).
DOCUMENTED = {
    "hausser_cell_type": (
    ...
    "hull_cell_type": (
        101, "103 labelled units; Golgi cells (n = 2) dropped by the n < 5 rule"),
...
def main() -> int:
    ...
    for dataset in sorted(os.listdir(DATASETS_DIR)):
        ...
        expected, reason = DOCUMENTED.get(dataset, (n_labels, None))
        bad = {k: v for k, v in caches.items() if v != expected}

        if bad:
            for method, n in sorted(bad.items()):
                problems.append(
                    f"{dataset}: {method} holds {n} pooled predictions, but the "
                    f"shipped dataset supports {expected}"
