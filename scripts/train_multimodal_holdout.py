...
# Tech / layer conditioning constants. The try/except mirrors the transductive
# script so the holdout script degrades gracefully if these symbols aren't
# present in the dataloading module.
try:
...
def split_by_animals(metadata_df, train_animals, test_animals, grouping_col='specimen_id'):
    """Split data indices based on animal/session IDs.

    Args:
        metadata_df: DataFrame with a grouping column (specimen_id for Allen,
                     subject for IBL).
        train_animals: IDs assigned to the training set.
        test_animals: IDs assigned to the test set.
        grouping_col: Column name used for splitting (default: 'specimen_id').
    """
    train_indices = metadata_df[metadata_df[grouping_col].isin(train_animals)].index.tolist()
    test_indices = metadata_df[metadata_df[grouping_col].isin(test_animals)].index.tolist()
...
def main():
    ...
    parser.add_argument("--acg_weight", type=float, default=1.0)
    ...
    parser.add_argument("--output-dir", "--output_dir", type=str, default=None,
                        help="Root output directory for results and checkpoints. "
                             "Defaults to ../results_hippie_holdout/<dataset>/fold_<N>/config_<cfg>_zdim_<z>_B_<b>. "
                             "Set this to control where results land for S3 upload.")
    ...
    # E1 conditioning-ladder flags (cond3 / cond4) — mirrors train_multimodal_transductive.py
    parser.add_argument("--use-tech-conditioning", action="store_true",
                        help="E1-cond3: replace the source_id label with a recording-technology "
                             "label (neuropixels/silicon_probe/juxtacellular/tetrode). "
                             "The existing source_embedding is reused with a smaller vocabulary.")
    parser.add_argument("--use-layer-conditioning", action="store_true",
                        help="E1-cond4: add a cortical/cerebellar layer conditioning embedding. "
                             "Loads layer_labels.csv if present; falls back to unknown (0) otherwise.")

    parser.add_argument("--seed", type=int, default=42,
                        help="Random seed (seeds python/numpy/torch/CUDA and DataLoader workers via pl.seed_everything).")
    args = parser.parse_args()

    # Resolve output directory early so checkpoint paths can reference it.
    # This is set BEFORE z_dim is resolved because the canonical subdir name
    # embeds z_dim; we'll finalize after z_dim is known.
    _output_dir_override = args.output_dir  # may be None

    # Resolve total z_dim from per-modality default unless an explicit override was passed.
    # train_multimodal_holdout always uses the 3 modalities {wave, isi, acg}, so we
    ...
    # Datasets supported for holdout (animal/session grouped CV)
    HOLDOUT_SUPPORTED = [
    ...
    if not any(dataset.startswith(prefix) for prefix in ["allen_", "Allen_", "ibl_", "IBL_"]):
        raise ValueError(
            f"This script supports Allen and IBL datasets with animal/session metadata. "
            f"Got: {dataset}. Supported datasets: {HOLDOUT_SUPPORTED}"
        )

    # Standardized animal grouping column.  Both Allen and IBL metadata
    # files now have an 'animal_id' column (added 2026-04-12) that aliases
    # specimen_id (Allen) / subject (IBL).  Fall back to the dataset-specific
    # column name if animal_id is missing (backwards compatibility).
    GROUPING_COL = "animal_id"
    ...
    if GROUPING_COL not in metadata_df.columns:
        # Fallback: try dataset-specific column names
        is_ibl = dataset.startswith("ibl_") or dataset.startswith("IBL_")
        fallback = "subject" if is_ibl else "specimen_id"
        if fallback in metadata_df.columns:
            print(f"[holdout] 'animal_id' not in metadata; falling back to '{fallback}'")
            GROUPING_COL = fallback
        else:
            raise ValueError(
                f"Expected column '{GROUPING_COL}' (or '{fallback}') in metadata.csv for dataset '{dataset}'. "
                f"Available columns: {list(metadata_df.columns)}"
            )
    # Build per-cell animal_id array from the FULL metadata (before filtering
    # by min_cells_per_animal). This array has one entry per CSV row, matching
    # the data arrays loaded below. train_indices/test_indices are original-CSV
    # row positions, so _all_animal_ids must also be indexed by original row
    # position. It is later filtered alongside data arrays via keep_mask.
    _all_animal_ids = metadata_df[GROUPING_COL].values.copy()

    animal_counts = metadata_df[GROUPING_COL].value_counts()
    valid_animals = animal_counts[animal_counts >= args.min_cells_per_animal].index.tolist()
    metadata_df = metadata_df[metadata_df[GROUPING_COL].isin(valid_animals)]
    ...
    train_indices, test_indices = split_by_animals(
        metadata_df, train_animals, test_animals, grouping_col=GROUPING_COL
    )
    ...
    acg_path = find_dataset_file(dataset, "acg.csv")
    supervised_acg = pd.read_csv(acg_path).to_numpy() if os.path.exists(acg_path) else np.zeros_like(supervised_isi)
    ...
    #
    # _unlabeled_keep and _rare_keep are stored as persistent index arrays so
    # that the same row drops can be applied to super_regions.csv below.
    _unlabeled_keep = np.arange(len(supervised_wf), dtype=np.intp)  # default: keep all
    _rare_keep_mask = None  # set below if any rare classes are dropped
    try:
        ...
        if n_dropped > 0:
            ...
            supervised_acg = supervised_acg[keep_mask]
            supervised_labels = supervised_labels[keep_mask]
            _all_animal_ids = _all_animal_ids[keep_mask]
            ...
            train_indices = [int(new_index_for_old[i]) for i in train_indices if i < n_mask and keep_mask[i]]
            test_indices = [int(new_index_for_old[i]) for i in test_indices if i < n_mask and keep_mask[i]]
            ...
            _unlabeled_keep = np.where(keep_mask)[0].astype(np.intp)
        ...
        # filter in train_multimodal_transductive.py — mandatory per the task
        # doc's "Dataset Filtering Rules" section. Holdout previously skipped
        # this because Allen/IBL brain-region classes are all large; defensive
        # here for DANDI and future datasets.
        ...
        if rare_encoded:
            ...
            _rare_keep_mask = keep_mask_rare  # persist for super_regions filter
            ...
            supervised_acg = supervised_acg[keep_mask_rare]
            supervised_labels = supervised_labels[keep_mask_rare]
            _all_animal_ids = _all_animal_ids[keep_mask_rare]
            # Re-map train/test indices again (same pattern as unlabeled filter above).
            # Guard against out-of-bounds indices from prior filter stage.
            n_rare = len(keep_mask_rare)
            new_index_for_old = np.cumsum(keep_mask_rare) - 1
            train_indices = [int(new_index_for_old[i]) for i in train_indices if i < n_rare and keep_mask_rare[i]]
            test_indices = [int(new_index_for_old[i]) for i in test_indices if i < n_rare and keep_mask_rare[i]]
            # Re-fit encoder so labels remain sequential 0..K-1.
    ...
    # Without the flag, super_regions.csv is ignored even if present — this lets
    # us run Allen both with and without conditioning for the ablation.
    ...
    if args.use_region_conditioning:
        ...
        for fname in ("super_regions.csv", "layer_labels.csv"):
            try:
                conditioning_file = find_dataset_file(dataset, fname)
        ...
        if conditioning_file is None:
            print(f"[region-conditioning] No conditioning file found for {dataset} — disabling")
        else:
            try:
                super_regions_df = pd.read_csv(conditioning_file)
                _raw_super_regions = super_regions_df[super_regions_df.columns[0]].values
                # Apply the same two-stage row filter used for labels/waveforms above
                # so that all arrays stay aligned row-for-row.
                _raw_super_regions = _raw_super_regions[_unlabeled_keep]
                if _rare_keep_mask is not None:
                    _raw_super_regions = _raw_super_regions[_rare_keep_mask]
        ...
        print(f"--use-region-conditioning not set; skipping conditioning files for {dataset}")
    ...
    if args.use_tech_conditioning:
        tech_name = DATASET_TECHNOLOGY.get(dataset, "silicon_probe")
        tech_id = TECHNOLOGY_IDS.get(tech_name, 1)
        num_tech_types = len(TECHNOLOGY_IDS)
        _tech_conditioning_enabled = True
        print(f"[tech-conditioning] dataset={dataset} → tech={tech_name} (id={tech_id}), "
    ...
    if args.use_layer_conditioning:
        try:
            layer_path = find_dataset_file(dataset, "layer_labels.csv")
            raw_layer_df = pd.read_csv(layer_path)
            raw_layer = raw_layer_df[raw_layer_df.columns[0]].values
            # Apply the same row filters used for labels/waveforms
            raw_layer = raw_layer[_unlabeled_keep]
            if _rare_keep_mask is not None:
                raw_layer = raw_layer[_rare_keep_mask]
            ...
            print(f"[layer-conditioning] Loaded layer_labels.csv: "
                  f"{len(np.unique(supervised_layer_ids))} layers for {dataset}")
        except FileNotFoundError:
            print(f"[layer-conditioning] No layer_labels.csv for {dataset} — using unknown (0)")
    ...
    # Split layer_ids if layer conditioning enabled
    ...
    all_dataset_files = {
        ...
        "ibl_brainwide": 9,
        "ibl_brainwide_good": 9,  # same source ID as ibl_brainwide (good-units subset)
    }

    num_sources = max(all_dataset_files.values()) + 1
    # When tech conditioning is active, source_embedding uses tech vocabulary (4 types)
    # instead of the per-dataset ID vocab.
    ...
    _pretrain_src = tech_id if _tech_conditioning_enabled else all_dataset_files[dataset]
    source_labels_train = _pretrain_src * np.ones(len(label_train))

    # Create pretraining datasets (only train animals)
    train_data_dict = {
        "wave": wf_train,
        "isi": isi_train,
        "acg": acg_train
    ...
    # Stack labels: (class=0, source [, super_region [, layer]])
    _pretrain_cols = [np.zeros_like(label_train), source_labels_train]
    if super_region_train is not None:
        _pretrain_cols.append(super_region_train)
    if layer_train is not None:
        if super_region_train is None:
            _pretrain_cols.append(np.zeros_like(label_train))  # placeholder sr column
        _pretrain_cols.append(layer_train)
    train_labels_stacked = np.vstack(_pretrain_cols).T
    ...
    # Determine canonical results directory (used for all checkpoints + outputs).
    _default_results_root = (
        f"../results_hippie_holdout/{dataset}/fold_{args.holdout_fold}"
        f"/config_{args.config}_zdim_{args.z_dim}_B_{args.beta}"
    )
    _results_root = _output_dir_override if _output_dir_override else _default_results_root
    os.makedirs(_results_root, exist_ok=True)

    # Save pretrained model
    joint_path = os.path.join(_results_root, "joint_model.ckpt")
    ...
    # Layer embedding is newly added in supervised phase — skip to avoid shape mismatch
    if _layer_conditioning_enabled:
        keys_to_remove = [k for k in joint_seq["state_dict"].keys()
                         if k.startswith("model.layer_embedding.") or
                            k.startswith("model.fusion_encoder.") or
                            k.startswith("model.decoder_fcs.")]
        for key in keys_to_remove:
            joint_seq["state_dict"].pop(key)
    ...
    _sup_src = tech_id if _tech_conditioning_enabled else all_dataset_files[dataset]
    source_labels_train_supervised = _sup_src * np.ones_like(label_train)

    # Stack labels: (class, source [, super_region [, layer]])
    _sup_cols = [label_train, source_labels_train_supervised]
    if super_region_train is not None:
        _sup_cols.append(super_region_train)
    if layer_train is not None:
        if super_region_train is None:
            _sup_cols.append(np.zeros_like(label_train))  # placeholder sr column
        _sup_cols.append(layer_train)
    train_labels_supervised_stacked = np.vstack(_sup_cols).T
    ...
    supervised_path = os.path.join(_results_root, "supervised_model.ckpt")
    ...
    _test_src = tech_id if _tech_conditioning_enabled else all_dataset_files[dataset]
    source_labels_test = _test_src * np.ones_like(label_test)

    # Stack labels: (class, source [, super_region [, layer]])
    _test_cols = [label_test, source_labels_test]
    if super_region_test is not None:
        _test_cols.append(super_region_test)
    if layer_test is not None:
        if super_region_test is None:
            _test_cols.append(np.zeros_like(label_test))  # placeholder sr column
        _test_cols.append(layer_test)
    test_labels_stacked = np.vstack(_test_cols).T
    ...
    # Ensure results directory exists (reuse the root determined above)
    ...
    # Prepare animal IDs early for the embedding CSV save.
    test_animal_ids = [_all_animal_ids[i] for i in test_indices[:len(test_embeddings)]]
    train_animal_ids = [_all_animal_ids[i] for i in train_indices[:len(train_embeddings)]]
    ...
    train_emb_df['animal_id'] = train_animal_ids
    ...
    test_emb_df['animal_id'] = test_animal_ids
