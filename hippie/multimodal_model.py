...
try:
    # Package-style import (used by `from hippie.multimodal_model import ...`)
    from .backbones import ResNet18Enc, ResNet18Dec
except ImportError:
    # Flat-dir import (used by training scripts that add hippie/ to sys.path)
    from backbones import ResNet18Enc, ResNet18Dec
...
class CVAEConfig:
    ...
    # and lisberger_labeled_cell_type only (the first two datasets we ablated).
    beta: float = 0.9
...
class ExperimentConfigs:
    ...
    def conditional_decoder_only():
        ...
        Configurations that pass the class embedding through the encoder require careful
        train/test alignment because the embedding is set to zero at evaluation time. Here
        the encoder path is class-agnostic by construction, so the encoder sees the same
        (zero) class embedding during training and evaluation and the two phases match
        exactly. The class embedding is still active in the decoder during training, so
        reconstruction is class-conditioned and the latent z is free from the burden of
        encoding class identity."""
        return CVAEConfig(
            use_source_embedding=True,
            use_class_embedding=True,
    ...
    def full_architecture_heavy_reg():
        ...
        Tests whether stronger train/test alignment regularizers are sufficient on their own
        to keep the encoder's behavior consistent across the with-label (training) and
        masked-label (evaluation) settings."""
        return CVAEConfig(
            use_source_embedding=True,
            use_class_embedding=True,
            use_fusion_encoder=True,
    ...
    def class_decoder_source_bn_aug_reg():
        ...
        Why this is the production config:

        1. Encoder/decoder asymmetry (rung 3 → 4 on the ladder): encoder_uses_class_embedding=False
           makes the encoder path class-agnostic by construction. An alternative configuration
           (full_architecture) routes the class embedding through the encoder and relies on
           regularizers to align the with-label (training) and masked-label (evaluation) settings;
           at 5-fold on the two tuning datasets that approach produces mean balanced accuracy
           0.468 (cellexplorer 0.460, hausser 0.475). The asymmetric configuration here hits
           0.635 (cellexplorer 0.557, hausser 0.713) — a +0.17 mean-accuracy improvement just
           from moving class conditioning to the decoder and keeping the same regularizers.

        2. Hausser is the representative tuning dataset. Dataset-level cosine distance analysis
           shows hausser sits 2nd-closest to the
