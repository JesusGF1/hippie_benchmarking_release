"""Supplemental Figure 3 — ISI+ACG dataset profiles.
...
Emits a 2-row panel per dataset:
    Row 0: mean waveform, mean ISI distribution, mean ACG distribution
    Row 1: cell-type distribution bar chart, dataset summary table

Outputs:
    figures/27_figure_3_supp_isiacg_profiles/<dataset>_supp_profile.{svg,png}

Run as:
    python analysis/figure_3_supp_isiacg_profile.py            # both
    python analysis/figure_3_supp_isiacg_profile.py 473        # 000473 only
    python analysis/figure_3_supp_isiacg_profile.py 955        # 000955 only
"""
...
DATASETS = {
    "473": {
        "key": "dandi_000473_cell_type",
        "display": "Calvigioni — DANDI 000473",
        "species": "Mouse",
        "brain_region": "Prefrontal cortex",
        "recording_tech": "Neuropixels",
        "class_order": ["Excitatory", "Inhibitory"],
        "n_subjects": 25,
    },
    "955": {
        "key": "dandi_000955_cell_type",
        "display": "Ramachandran — DANDI 000955",
        "species": "Rat",
        "brain_region": "Somatosensory cortex",
        "recording_tech": "32-channel NeuroNexus electrodes",
        "class_order": ["Excitatory", "Inhibitory"],
        "n_subjects": 1,
    },
}
...
def load(dataset_key: str, class_order: list[str]):
...
def _plot_distribution(ax, labels: pd.Series, classes: list[str]) -> None:
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(classes, rotation=20, ha="right", fontsize=7)
...
def _plot_summary_table(ax, info: dict, n_cells: int, classes: list[str],
    ...
    subj_str = str(info["n_subjects"]) if info["n_subjects"] else "N/A"
    cell_text = [
...
def make_profile(which: str) -> None:
    ...
    for ax in (ax_isi, ax_acg, ax_dist, ax_meta):
    ...
    stem = OUT_DIR / f"{key}_supp_profile"
