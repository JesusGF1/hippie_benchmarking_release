"""Figure 5 panel A — cross-species method comparison bars (Lisberger→Hull).

KNN balanced accuracy for Lisberger (macaque) → Hull (mouse) transfer,
six methods including VAE baselines.

Outputs:
    figures/29_figure_5_cross_species/panel_a_method_bars.{svg,png}
"""
...
# (label, s3_prefix, pred_rel_path, true_col, pred_col, color)
METHODS = [
    ("HIPPIE",              "hippie-cross-dataset",
     "predictions/predictions.csv", "true_label", "predicted_label", _BLUE),
    ("HIPPIE\n+Pretraining", "hippie-cross-dataset-allpretrain",
     "predictions/predictions.csv", "true_label", "predicted_label", _BLUE),
    ("NEMO",                "nemo-cross-dataset",
     "predictions/cross_dataset/cross_dataset_predictions.csv", "true", "pred", _BLUE),
    ...
    ("PhysMAP\n(WNN)",      "physmap-cross-dataset",
     "predictions/physmap_cross_dataset_results.csv", "label", "prediction", _BLUE),
...
def main() -> None:
    ...
    for xi, (label, prefix, pred_rel, true_col, pred_col, color) in enumerate(METHODS):
        ba = _load_ba(prefix, pred_rel, true_col, pred_col)
        if ba is not None:
            ax.bar(xi, ba, width=0.65, color=color, edgecolor="white", linewidth=0.8, zorder=2)
            ax.text(xi, ba + 0.015, f"{ba:.2f}", ha="center", va="bottom",
                    fontsize=7, fontweight="bold", color="#333")
        else:
            ax.bar(xi, 0, width=0.65, color=color, edgecolor="white", linewidth=0.8, zorder=2,
                   alpha=0.3)
    ...
    for ext in ("svg", "png"):
        p = OUT_DIR / f"panel_a_method_bars.{ext}"
